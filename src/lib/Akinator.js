...
module.exports = class Akinator {
  ...
  */
  async start() {
    ...
    const { status, data } = await request(`${this.uri}/new_session?callback=${jQuery + new Date().getTime()}&urlApiWs=${this.urlApiWs}&partner=1&childMod=${this.childMode.childMod}&player=website-desktop&uid_ext_session=${this.uid}&frontaddr=${this.frontaddr}&constraint=ETAT<>'AV'&soft_constraint=${this.childMode.softConstraint}&question_filter=${this.childMode.questionFilter}`);

    if (status !== 200 || !data || data.completion !== 'OK' || !data.parameters || !data.parameters.step_information.question) {
  ...
  async step(answerId) {
    ...
    const { status, data } = await request(`${this.uri}/answer_api?callback=${jQuery + new Date().getTime()}&urlApiWs=${this.urlApiWs}&childMod=${this.childMode.childMod}&session=${this.session}&signature=${this.signature}&step=${this.currentStep}&answer=${answerId}&frontaddr=${this.frontaddr}&question_filter=${this.childMode.questionFilter}`);

    if (status !== 200 || !data || data.completion !== 'OK' || !data.parameters || !data.parameters.question) {
      akinatorAPIErrors(data, this.region);
      return;
    }
  ...
  async back() {
    ...
    const { status, data } = await request(`${this.urlApiWs}/cancel_answer?&callback=${jQuery + new Date().getTime()}&session=${this.session}&childMod=${this.childMode.childMod}&signature=${this.signature}&step=${this.currentStep}&answer=-1&question_filter=${this.childMode.questionFilter}`);
  ...
  async win() {
    ...
    const { status, data } = await request(`${this.urlApiWs}/list?callback=${jQuery + new Date().getTime()}&signature=${this.signature}${this.childMode === true ? `&childMod=${this.childMode}` : ''}&step=${this.currentStep}&session=${this.session}`);

    if (status !== 200 || !data || data.completion !== 'OK' || !data.parameters || !data.parameters.elements) {
      akinatorAPIErrors(data, this.region);
      return;
    }
