...
module.exports = async (region) => {
  ...
  const result = await request(`https://${uri}/new_session?callback=${jQuery + new Date().getTime()}&urlApiWs=https://${urlApiWs}/ws&partner=1&player=website-desktop&uid_ext_session=${uid}&frontaddr=${frontaddr}&constraint=ETAT%%3C%%3E%%27AV%%27&constraint=ETAT<>'AV'`);
  const { body, statusCode } = result;

  if (!statusCode || statusCode !== 200 || !body || body.completion !== 'OK' || !body.parameters || !body.parameters.step_information.question) return akinatorAPIErrors(body, region);
