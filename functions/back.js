...
module.exports = async (region, session, signature, answerId, step) => {
    ...
    const opts = {
        ...
        uri: `https://${id}/ws/cancel_answer?callback=&session=${session}&signature=${signature}&step=${step}&answer=-1`,
        headers: {
    ...
    if (json.completion === 'OK') {
        ...
        throw new Error('Unknown error has occurred. Server response: ' + json.completion);
    }
};
