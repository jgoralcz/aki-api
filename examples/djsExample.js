...
module.exports = {
     ...
     */
    async run(bot, prefix, message, args, userInfo) {
        ...
        if(args.length >= 1) {
            ...
            let i = this.regions.findIndex( (reg) => testRegion === reg);
            ...
            setTimeout(async () => {
                ...
                if (nextInfo.progress >= 78 && loop > 3 || nextInfo.nextStep >= 79) {
                    ...
                    if (win.answers != null && win.answers.length > 0) {
                        ...
                        myMessage = await bot.editMessage(myMessage, '', {embed: embed});
     ...
     */
    help: async function(bot, prefix, message, args) {
